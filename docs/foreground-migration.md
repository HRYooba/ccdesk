...
- メニューが唯一の入口になったので、**サイドバーにフォーカスがあるときの `←` を
  「その行のメニュー」**にした（`→` / Enter の「開く」の対）。行の種類で分けない
  ＝ セッション行・見出し行・グルーピング行のどれでも同じ規則で開き、メニューを
  持たない行では無反応。**予約キーには足していない**ので、端末側にフォーカスが
  あれば `←` は今までどおり claude へ渡る（ペイン移動は `Alt+←`）
...
- **マウスで押せるものはキーボードでも押せる**に揃えた。サイドバー本体ではなく
  ...
  位置に同じメニューを開く。選択位置は**一覧の行とアカウント行を 1 つで表す型**
  （`app.rs` の `Selection`）が持つ ＝ 「行 index + アカウントか」の 2 つに分けて

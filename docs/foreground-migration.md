...
- **マウスで押せるものはキーボードでも押せる**に揃えた。サイドバー本体ではなく
  フッターに描かれるアカウント行も `↑↓` の行き先になる。位置は
  **一覧の行とアカウント行を 1 つで表す型**
  ...
  （**どちらもマウスの当たり判定と同じ計算**）。
  なおアカウント行が開いていたメニューはフェーズ9で撤去された（下記）
...
### フェーズ9: アカウント切り替えを撤去する（済）

**この段階の本質は「正しく作れないと分かった機能を、確率で誤魔化さずに畳んだこと」。**
残したのは**今サインインしているアカウントの表示**（フッターの 1 行）だけで、
`register current` / `switch` / `unregister` と `~/.ccdesk/accounts.json` は消えた。

#### なぜ作れなかったか（同じ試みを繰り返さないために）

**判断材料が揃っていない。** `claude auth status --json` が答える email の出所は
`~/.claude.json` の `oauthAccount` ＝ **遅延取得のキャッシュ**で、
その更新契機が ccdesk からは見えない。実測で確かめたこと:

- `claude auth status` 自体は `.claude.json` を**書かない**（6 回連続実行で確認）
  ＝ 聞き直しても更新を促せない
- `.claude.json` は動いている claude が**約 2.8 秒おきに書き換え続けている**
  ＝ mtime は「答えが新しくなったか」を何も語らない

つまり「**今このトークンは誰のものか**」を確実に知る方法が無い。それが分からないまま
保管へ書けば、**別アカウントのトークンをこの email の保管へ入れる**。refreshToken は
使い捨て（one-time use）なので、それは**復旧不能な破壊**になる（保管された側は
元のトークンを二度と得られない）。実機でも保管が繰り返し汚染され、ユーザーが何度も
手で復旧する事態になった。

**確率を下げる対策は正しさを保証しない。** 試したのは 2 つ:

- **変化の窓**（`CREDENTIALS_SETTLE` ＝ 認証情報が動いてから 30 秒は書かない）。
  遅れているのは claude 側のキャッシュで、その更新契機は上のとおり見えないので
  「30 秒待てば必ず正しい」という保証はどこにも無い。実機のログには、遅れが解けるまで
  30 秒以上かかったと読める並びが残っていた
- **保管の自己修復**（同じ refreshToken を持つ別の保管を見つけたら剥がす）。
  これは汚染が入った後で行き止まりにしないための手当てであって、汚染を防ぐものではない

どちらも「間違える確率を下げる」だけで、**間違えたときの被害が復旧不能**である以上、
機能として成立しない。だから畳んだ。

#### 何を残したか

- **アカウント行の表示は残す**（`poll.rs` の `parse_account` → フッターの 1 行）。
  読むのは `claude auth status --json` と `.claude.json` の表示名だけで、
  `~/.claude/.credentials.json` は**中身を読まない**（`metadata()` だけを
  「行を取り直す契機」の signal に使う ＝ `poll.rs` の `AuthWatch`）
- `AccountStatus::LoggedIn` が持つのは**表示ラベルだけ**になった。email は
  保管のキーとしてしか要らなかったので、持ち回る先が消えた
- **アカウント行は選択・ホバーできるが押しても何も起きない行**として残る。
  「何も起きない」の答えは `selected_enter` が `None` を返す 1 箇所にあり、
  更新の無い版行（`SidebarRow::Inert`）と**同じ 1 つの判断**を共有する
  （`SidebarPos::Account` のままなのは、この行がフッターに描かれて
  `sidebar_rows` の index を持たないため ＝ `Inert` へは寄せられない）
- **保管ファイルは起動時に一度だけ消す**（`lib.rs` の `purge_account_store`。
  呼ぶのは `main` の起動列だけ ＝ `enable_error_log` と同じ扱いで、テストの
  実行ファイルは構造上そこへ到達しない）。ロックと書きかけの `.tmp`
  （**中身はトークン**）も一緒に片付け、消したことを `error.log` へ 1 行残す

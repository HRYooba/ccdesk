...
/// ペインが保持する画面外の行数。**代替画面を使わない子（codex）は履歴を
/// 通常画面へ流すので、戻り先を持てるのは端末側 ＝ ccdesk だけ**
/// （実測: codex が起動時に立てるのは 9001/1004/2004/2026 のみで、
/// マウス報告も 1049 も無い ＝ ホイールは端末のスクロールバックが受ける前提）。
/// 代替画面 + マウス報告で自前スクロールする子（claude）には使われない
/// （vt100 の代替グリッドはスクロールバックを持たない）。
///
/// 1 行 = 桁数 × 32B（vt100 の Cell）なので、200 桁ならセッションあたり最大
/// 約 13MB。実際に流れた分しか確保しない

...
// 会話の発言（**非公開**）
//
// 縮退: 型名やキーが変わると `ccdesk read` が何も返さなくなる（`--screen` は
// vt100 の写しなので影響を受けない ＝ 読む手段は残る）。
// ---------------------------------------------------------------------------

/// 打鍵を運ぶ行の型名。**道具の結果も同じ型名で並ぶ**ので、型名だけでは
/// 発言と区別できない（本文ブロックの有無で分ける ＝ [`TEXT_BLOCK`]）
pub(crate) const USER_RECORD: &str = "user";
/// agent の答えを運ぶ行の型名
pub(crate) const AGENT_RECORD: &str = "assistant";
/// 発言の中身が入っている入れ子
pub(crate) const MESSAGE_KEY: &str = "message";
/// 発言の中身（文字列 1 本、またはブロックの配列）
pub(crate) const CONTENT_KEY: &str = "content";
/// ブロックの配列のうち、**本文を運ぶブロックの型名**。
/// 道具の呼び出し（`tool_use`）・結果（`tool_result`）・思考（`thinking`）は
/// これに当たらないので落ちる
pub(crate) const TEXT_BLOCK: &str = "text";

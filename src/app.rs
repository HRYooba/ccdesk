...
pub(crate) fn run(terminal: &mut ratatui::DefaultTerminal, app: &mut App) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if app.last_scan.elapsed() > SCAN_INTERVAL {
            ...
            //
            // **周期のたびに読み直す**（`hooks_changed` は「早く読む」ための合図で
            // あって、「読む唯一の条件」ではない）。変わった周だけ読んでいた頃、
            // 見え方の変化を 1 度取りこぼすと**次に誰かが書くまで古い写しを見続け**、
            // その間に撃つべき通知が撃たれずに溜まった ＝ 無関係な別セッションの
            // hook が書いた瞬間にまとめて出る（追跡ログの実測で 252 件中 27 件。
            // 最長 5 分半）。読み直しは 1 ファイルの JSON パース 1 回で、
            // 2 秒周期で払う値段としては安い ＝ **取りこぼしを設計から消す方を採る**

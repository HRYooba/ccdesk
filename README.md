...
- **Version rows** at the top of the sidebar — `ccdesk vX.Y.Z` and `claude vX.Y.Z` — each with a
  ...
  `⟳ ccdesk vX.Y.Z          update` and **clicking anywhere on it runs the update**
  (`updating…` while it runs, then `restart`). The marker column stays reserved when you are
  up to date, so nothing shifts sideways when an update appears. ccdesk's own check runs once
  per launch (no polling); the download is verified by SHA-256 before anything is replaced.
  **Both updates apply on the next launch** — the running ccdesk and any live claude session
  keep the version they started with. A native `claude` install also auto-updates in the
  background by default, so the claude row may clear itself without you doing anything
